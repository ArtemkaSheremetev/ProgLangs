...
struct FnEmitter {
    ...
    int genLoadVar(const std::string& name) {
        int dst = alloc();
        auto it = varLabel.find(name);
        if (it == varLabel.end()) {
            emitLiImm(dst, 0, "unknown var " + name);
            return dst;
        }
        // r6 = &var; load dst, r6
        emitLiDataLabel(6, it->second, "&" + name);
    ...
    void genStoreVar(const std::string& name, int src) {
        auto it = varLabel.find(name);
        if (it == varLabel.end()) {
            emit(reg32::Mnemonic::jmp, {reg32::Operand::codeLabel(fnEntryLabel)}, "WARN: store unknown var " + name);
            return;
        }
        emitLiDataLabel(6, it->second, "&" + name);
...
CodeGenResult buildProgramImage(const CallGraph& cg) {
    ...
    for (const auto& fkv : cg.functionGraphs) {
        ...
        // ---- save args r0..r3 into variables (so VarExpr("n") works) ----
        for (size_t i = 0; i < g.args.size() && i < 4; ++i) {
            const std::string& aname = g.args[i];
            auto it = fe.varLabel.find(aname);
            if (it == fe.varLabel.end()) continue;

            fe.emitLiDataLabel(6, it->second, "&" + aname);
            fe.emit(reg32::Mnemonic::store, {FnEmitter::R(6), FnEmitter::R((int)i)}, "arg " + aname);
        }
